...
async def analyze_image(file: UploadFile = File(...), vol: str = Form(...)):
    try:
        ...
        # 1. 전처리 (코랩에서 성공했던 가장 단순하고 강력한 방식)
        ...
        target_regions = sorted(cnts, key=lambda x: np.mean(x[:, 0, 0]))

        if len(target_regions) < 2:
            return {"error": "구역 인식 실패. 테두리가 선명하게 보이게 찍어주세요."}
        ...
        for idx, region in enumerate(target_regions):
            ...
            # --- [강사님 황금 좌표 수치 + RC 5열 보정] ---
            if section_name == "RC":
                # 181~187번 ? 현상 해결: 왼쪽 마진과 열 간격을 키워 점을 오른쪽으로 이동
                left_margin = w * 0.083      
                current_col_spacing = w * 0.198 
